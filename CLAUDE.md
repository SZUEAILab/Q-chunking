...
| **OGBench** (视觉)   | `{env}-visual-singletask{ -taskN}-v0` | `cube-triple-play-visual-singletask-task2-v0`                                                                      |
...
视觉环境需在 agent 配置中指定 `encoder`（`impala`、`impala_small` 等），并通过 `--agent.encoder=impala_small` 传递。
...
默认 `--gpu_tasks=1`。压力测试详见 [docs/parallel_benchmark.md](docs/parallel_benchmark.md)。

| GPU                 | 推荐 `--gpu_tasks` | 最大实测 | 甜点分析 |
| ------------------- |:---:|:---:|---------|
| RTX 6000 Ada (48GB) | 6 | 8 (N=10 崩溃) | TanhNormal: 558 it/s @N=8, 4.9× |
| RTX 4090 (24GB)     | **4** | 10 (全存活) | TanhNormal: 554 it/s @N=12; DS: 402 it/s @N=10 |
...
用法详见 [docs/schedule.md](docs/schedule.md)。核心流程：
