...
设连续动作 $a\in[-1,1]^D$ 可划分为空间位移 $v=a_{1:d}$ 和标量控制项 $c=a_{d+1:D}$。定义方向-速度坐标：
...
对应到上一节的图像语言：baseline 的 $\sigma_v$ 先定义 Cartesian 空间里的蓝色采样置信椭球，再被动地诱导出一个随 $l=\|\mu_v\|$ 改变的红色方向视锥；DS 则直接在方向和速度上定义探索。方向维度的 $\sigma_u$ 固定时，方向探索视锥就固定；速度维度的 $\sigma_s$ 只影响速度探索，不会顺带改变方向。

### 3.2 速度维度：为什么使用 log-speed

方向-速度分解里，方向 $u$ 描述“往哪里走”，速度或步长 $m=\|v\|$ 描述“走多远”。如果直接在原始速度 $m$ 上加噪声，

$$
m' = m + \epsilon_m,
$$

那么同一个 $\epsilon_m$ 对小步长和大步长的含义完全不同。例如 $m=0.1$ 时加 $0.05$ 是 50% 的相对变化；$m=1.0$ 时加 $0.05$ 只是 5% 的相对变化。因此，原始速度空间里的加性噪声更像固定绝对位移偏移，而不是尺度一致的速度探索。

为了解决这个问题，可以把速度写到对数空间：
...
这样，latent 中速度方向的加性变化对应原始速度中的乘性变化：
...
例如 $\epsilon_s=\log 1.1$ 时，无论当前速度是 $0.1$ 还是 $1.0$，扰动后的速度都是原来的 $1.1$ 倍。这使得速度维度的 $\sigma_s$ 更容易解释为“相对步长探索强度”，而不是某个固定长度单位上的绝对噪声。
